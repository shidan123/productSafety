            ...
            <form action="role/update.do" method="post" class="main_form">
               <input type="hidden" name="id" value="${roleInfo.id }">
                ...
                <div class="input_info">
                    <input type="text" class="width200" id="name" name="name" value="${roleInfo.name }" onblur="validateName();" />
                    <span class="required">*</span>
                    <div id="nameDiv" class="validate_msg_medium ">不能为空，且为10长度的字母、数字和汉字的组合</div>
                </div>                    
                ...
                <div class="input_info_high">
                    <div class="input_info_scroll">
                        <ul>
                            ...
                            <li><input type="checkbox" id="privilegeIds" value="${privilege.id }" name="privilegeIds" 
                        	<c:forEach items="${roleInfo.privileges }" var="rolePrivilege">
                        		<c:if test="${rolePrivilege.id==privilege.id }">
                        		checked="checked"
                        		</c:if>
                        	</c:forEach>
                            />${privilege.name }</li>
